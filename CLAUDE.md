...
- 菜品与 `items` 无关，不复用到期/消耗的条目模型：菜品没有时间维度，也没有 ok/warning/danger 状态。新增字段时不要往 `items` 上挂。
- `recipe_ingredients.quantity` 是自由文本（"2 个"、"适量"、"300g"），`NOT NULL DEFAULT ''`，空串表示未记录，展示层回退为「适量」。不要改成数字加单位，家庭用量本来就不精确。
- 用料和步骤是从属行，保存时**整组全量替换**（PATCH 内先 DELETE 再按 `position` 重新 INSERT），不做逐行 diff。因此前端表单必须提交完整列表，漏传会清空。
- 列表查询只带用料和步骤条数，步骤正文只在详情页查询，避免列表页拉取无用数据。
- 搜索必须同时匹配菜名和食材名，这是「家里有茄子能做什么」的唯一入口；因此食材名的一致性很重要，录入时提供已有食材名的自动补全。
- 菜单模块不使用 `--lt-ok/warn/danger`。这些状态色在本项目专门表达时间紧迫感，借用会稀释首页状态色的信号强度。
- 菜品不做归档，删除即删除（用料和步骤靠外键级联清理）。

### 推送通知

- 推送入口在主页账号菜单（`AccountMenu`）里，是同一个菜单项：未订阅时点击直接触发订阅流程，菜单保持打开、不跳转页面；订阅成功后该项变成跳转 `/webhook` 的链接。取消订阅只能在 `/webhook` 页面完成，主页不提供取消订阅的交互，避免状态语义膨胀。
- iOS 未添加到主屏幕时，菜单里显示一条静态说明而不是可点击项——它不是操作，点了也没有结果。
...
- 菜单：新增、编辑（确认步骤未被全量替换清空）、删除、按食材搜索、详情页步骤展示。
