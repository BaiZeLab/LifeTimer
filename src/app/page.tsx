...
// ── Visual config ─────────────────────────────────────────────────────────────
// expired uses CSS var so dark mode can override the muted-slate color
...
// ── DayBadge — circular SVG arc progress ring ─────────────────────────────────
...
// ── StatusSummary — proportional bar chart ────────────────────────────────────
...
// ── OverflowMenu — dropdown via portal (bypasses overflow:hidden & transform) ──
...
// ── ConsumptionChart — improved SVG chart ─────────────────────────────────────
...
function ConsumptionCard({
  ...
  return (
    ...
    >
      ...
      {/* Step 1 — one log, needs second to start predicting */}
      {item.logCount === 1 && (
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/items/${item.id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: name.trim(), notes: notes || null,
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/items/${item.id}/renew`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ newExpireDate, newStartDate: newStartDate || item.expireDate, notes: notes || undefined }),
      });
...
function LogModal({ item, onClose, onSaved }: {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/items/${item.id}/logs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ value: num, recordedAt: new Date(recordedAt).toISOString(), notes: notes || undefined }),
      });
